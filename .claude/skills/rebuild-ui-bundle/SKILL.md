...
- The server caches the built HTML per process (`_dist_html()` in `ui/__init__.py`). If a long-running
  `uv run python -m mal_mcp.server` is up, **restart it** to pick up a rebuild. A *miss* is not cached,
  so building while the process runs takes effect on the next read.
